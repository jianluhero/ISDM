...
public class SampleCivilian extends AbstractSampleAgent<Civilian> {
	...
	public Registry getPreferredRegistry(Registry parent) {
		...
		propertyURNs.add(StandardPropertyURN.DELAY.toString());
		...
		if (me().getDelay() < time) {
			if (damage == 0 && buriedness == 0) {
				...
				if (runForRefuge) // Run for the refuge
					...
					// Logger.info("civ: "+me.getID().getValue()+
					// " destionation "+me().getDestination());
				...
				/*
				 * if (path != null) { Logger.info("Heading for a refuge");
				 * sendMove(time, path); return; } else {
				 * Logger.info("Moving randomly"); sendMove(time, randomWalk());
				 * return; }
				 */
			...
			Logger.info("Not moving: damage = " + damage + ", buriedness = "
					+ buriedness);
			sendRest(time);
		} else {
			Logger.debug("dealy---current time: " + time + " delay: "
					+ me().getDelay());
		}
	}
