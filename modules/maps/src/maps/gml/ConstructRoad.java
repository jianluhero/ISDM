...
public class ConstructRoad extends Component {
	...
	private double a=0.65;
	private double ep=0.0001;

	public double min(double v1, double v2, double v3) {
		return Math.min(Math.min(v1, v2), v3);
	}
	
	public void removePointinSameLine(List<Point2D.Double> points)
	{
		if(points.size()<=2)
			return;
		else {
			for(int i=0;i<points.size()-2;i++)
			{
				double angle=getAngle(
						new double[] { points.get(i + 1).getX(),
								points.get(i + 1).getY(), 0 },
						new double[] { points.get(i).getX(),
								points.get(i).getY(), 0 }, new double[] {
								points.get(i + 2).getX(),
								points.get(i + 2).getY(), 0 });
				if(Math.sin(angle)<=ep)
				{
					points.remove(i+1);
				}
			}
	...
	public Path2D.Double getPath(List<Point2D.Double> points) {
		...
		if (points.size() == 2) {
			...
			r1.add(p1);
			r2.add(p2);
			if (Math.abs(points.get(0).getX() - points.get(1).getX()) < ep) {

			} else {
				if ((points.get(0).getY() - points.get(1).getY())
						/ (points.get(0).getX() - points.get(1).getX()) == (p3.y - p1.y)
						/ (p3.x - p1.x)) {
					r1.add(p3);
					r2.add(p4);
				} else {
			...
			for (int i = 0; i < points.size() - 2; i++) {
				double length1 = getLineLength(new Line2D.Double(points.get(i),
						points.get(i + 1)));
				double length2 = getLineLength(new Line2D.Double(
				...
				double angle = getAngle(
						...
						new double[] { points.get(i).getX(),
								...
								points.get(i + 2).getX(),
								points.get(i + 2).getY(), 0 });
				double w = minLength * Math.sin(angle / 2)
						* Math.cos(angle / 2) * 2 *a;
				if(w<ep)
					w=road_Width;

				if (i == 0)
					roadWidth1 = w < road_Width ? w : road_Width;
				else {
					roadWidth1 = roadWidth2;
				}
				if (i == points.size() - 3)
					roadWidth2 = roadWidth1;
				else {
					length1 = getLineLength(new Line2D.Double(
							points.get(i + 1), points.get(i + 2)));
					length2 = getLineLength(new Line2D.Double(
							points.get(i + 2), points.get(i + 3)));
					minLength = length1 > length2 ? length2 : length1;
					angle = getAngle(new double[] { points.get(i + 2).getX(),
							points.get(i + 2).getY(), 0 }, new double[] {
							points.get(i + 1).getX(), points.get(i + 1).getY(),
							0 }, new double[] { points.get(i + 3).getX(),
							points.get(i + 3).getY(), 0 });
					w = minLength * Math.sin(angle / 2) * Math.cos(angle / 2)
							* 2 *a;
					if(w<ep)
						w=road_Width;
					roadWidth2 = min(roadWidth1, road_Width, w);
				}

				System.out.println(roadWidth1 + "  -----   " + roadWidth2);
				...
				double angle1 = getAngle(
						new double[] { points.get(i + 1).getX(),
								points.get(i + 1).getY(), 0 }, new double[] {
								p2.getX(), p2.getY(), 0 }, new double[] {
								points.get(i + 2).getX(),
								points.get(i + 2).getY(), 0 });
				double angle2 = getAngle(
						new double[] { points.get(i + 1).getX(),
								points.get(i + 1).getY(), 0 }, new double[] {
								p1.getX(), p1.getY(), 0 }, new double[] {
								points.get(i + 2).getX(),
								points.get(i + 2).getY(), 0 });
				if (angle1 > angle2) {
					innerP1 = p1;
				...
				Line2D.Double line = new Line2D.Double(points.get(i + 1),
						points.get(i + 2));
				Line2D.Double[] lines = getPerpendicularPoint(line, roadWidth2);
		...
		for (int i = 0; i < r2.size(); i++) {
			path.lineTo(r2.get(r2.size() - 1 - i).x,
					r2.get(r2.size() - 1 - i).y);
	...
	/**
	 * TODO: if two lines have very small angles, then if the gap is small than
	 * roadwidth need to remove that point
	 * 
	 * @param points
	 * @return
	 */
	public ArrayList<ArrayList<Point2D.Double>> getEvacutionRoutine(
	...
	public void paint(Graphics g) {
		...
		Point2D.Double p3 = new Point2D.Double(40.5, 200.5);
		Point2D.Double p4 = new Point2D.Double(140.5, 240.5);
		...
		p1 = new Point2D.Double(500.5, 500.5);
		p2 = new Point2D.Double(500.5, 600.5);
		p3 = new Point2D.Double(510.5, 500.5);
		p4 = new Point2D.Double(510.5, 600.5);
		p5 = new Point2D.Double(530.5, 400.5);
		p6 = new Point2D.Double(520.5, 500.5);
		g2.setColor(Color.green);
		g2.draw(new Line2D.Double(p1, p2));
		g2.draw(new Line2D.Double(p2, p3));
		g2.draw(new Line2D.Double(p3, p4));
		...
		ArrayList<Point2D.Double> a3 = new ArrayList<Point2D.Double>();
		a3.add(p1);
		a3.add(p2);
		a3.add(p3);
		a3.add(p4);
		...
		p1 = new Point2D.Double(600.5, 600.5);
		p2 = new Point2D.Double(600.5, 700.5);
		p3 = new Point2D.Double(500.5, 700.5);
		p4 = new Point2D.Double(710.5, 700.5);
		g2.setColor(Color.green);
		g2.draw(new Line2D.Double(p1, p2));
		g2.draw(new Line2D.Double(p2, p3));
		g2.draw(new Line2D.Double(p3, p4));
		ArrayList<Point2D.Double> a4 = new ArrayList<Point2D.Double>();
		a4.add(p1);
		a4.add(p2);
		a4.add(p3);
		a4.add(p4);
