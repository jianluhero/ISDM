...
public class GMLMap implements maps.Map {
	 ...
	 * 
	 * @param x
	 *            The X coordinate of the node in m.
	 * @param y
	 *            The Y coordinate of the node in m.
	...
	public void add(GMLObject object) {
		if (object instanceof GMLNode) {
			...
			throw new IllegalArgumentException("Don't know how to add "
	...
	/**
	 * Remove a node, any edges attached to the node and any shapes attached to
	 * those edges.
	 * 
	 * @param n
	 *            The node to remove.
	...
	public Collection<GMLObject> removeNode(GMLNode n) {
		...
		if (nodes.containsKey(n.getID())) {
			...
			Collection<GMLEdge> attached = new HashSet<GMLEdge>(
					getAttachedEdges(n));
	...
	public Collection<GMLObject> removeEdge(GMLEdge e) {
		...
		if (edges.containsKey(e.getID())) {
			...
			Collection<GMLShape> attached = new HashSet<GMLShape>(
					getAttachedShapes(e));
	...
	public void removeAllBuildings() {
		for (Map.Entry<GMLEdge, Collection<GMLShape>> entry : attachedShapes
				.entrySet()) {
	...
	/**
	 * Find out if this map has a real size or not. Maps with zero or one nodes
	 * do not have a real size.
	 * 
	 ...
	 * 
	 * @param conversion
	 *            The coordinate conversion to apply.
	 */
	 ...
	 * @param second
	 *            The 'end' node.
	 * @return A new GMLEdge with a unique ID or an existing edge. The returned
	 *         edge may be reversed with respect to first and second.
	 */
	public GMLEdge ensureEdge(GMLNode first, GMLNode second) {
		for (GMLEdge next : edges.values()) {
			if ((next.getStart().equals(first) && next.getEnd().equals(second))
					|| (next.getStart().equals(second) && next.getEnd().equals(
							first))) {
				return next;
			}
		}
	...
	public List<GMLDirectedEdge> apexesToEdges(List<GMLNode> apexes) {
		List<GMLDirectedEdge> edgeList = new ArrayList<GMLDirectedEdge>(
				apexes.size());
	 ...
	 */
	public Collection<GMLNode> getNodesInRegion(double xMin, double yMin,
	 ...
	 */
	public GMLEdge findNearestEdge(double x, double y,
			Collection<? extends GMLEdge> possible) {
		GMLEdge best = null;
		double bestDistance = Double.NaN;
	...
	public GMLShape findShapeUnder(double x, double y) {
		for (GMLShape next : allShapes) {
			if (GMLTools.coordsToShape(next.getUnderlyingCoordinates())
					.contains(x, y)) {
				return next;
			}
		}
	...
	/**
	 * Merge a pair of edges and form a new edge. The node and existing edges
	...
	public GMLEdge mergeEdges(GMLEdge edge1, GMLEdge edge2) {
		...
		if (!commonNode.equals(edge2.getStart())
				&& !commonNode.equals(edge2.getEnd())) {
			commonNode = edge1.getEnd();
		}
		if (!commonNode.equals(edge2.getStart())
				&& !commonNode.equals(edge2.getEnd())) {
			throw new IllegalArgumentException("Edges " + edge1 + " and "
					+ edge2 + " do not have a common node");
		}
		GMLNode start = commonNode.equals(edge1.getStart()) ? edge1.getEnd()
				: edge1.getStart();
		GMLNode end = commonNode.equals(edge2.getStart()) ? edge2.getEnd()
				: edge2.getStart();
		return ensureEdge(start, end);
	}

	/**
	 * Replace all references to a node with another node. This does not delete
	 * the old node from the map.
	 * 
	 * @param oldNode
	 *            The node to replace.
	 * @param newNode
	...
	public void replaceNode(GMLNode oldNode, GMLNode newNode) {
		List<GMLEdge> attached = new ArrayList<GMLEdge>(
				getAttachedEdges(oldNode));
		for (GMLEdge next : attached) {
	...
	/**
	 * Replace all references to an edge with another edge. This does not delete
	 * the old edge from the map. The two edges must have the same pair of start
	 * and end nodes but may be in different directions.
	 * 
	 * @param oldEdge
	 *            The edge to replace.
	 * @param newEdge
	...
	public void replaceEdge(GMLEdge oldEdge, GMLEdge newEdge) {
		if ((oldEdge.getStart() != newEdge.getStart() && oldEdge.getStart() != newEdge
				.getEnd())
				|| (oldEdge.getEnd() != newEdge.getStart() && oldEdge.getEnd() != newEdge
						.getEnd())) {
			throw new IllegalArgumentException(
					"oldEdge and newEdge do not share start and end nodes");
		}
		Collection<GMLShape> attached = new HashSet<GMLShape>(
				getAttachedShapes(oldEdge));
		for (GMLShape next : attached) {
			for (GMLDirectedEdge dEdge : next.getEdges()) {
				if (dEdge.getEdge() == oldEdge) {
					...
					GMLDirectedEdge replacement = new GMLDirectedEdge(newEdge,
							forward);
	...
	/**
	 * Insert a node into an edge. This method updates attached edges but does
	 * not delete the old edge.
	 * 
	 * @param edge
	 *            The edge to split.
	 * @param node
	...
	private GMLEdge crossEdges[];

	public void writePath(GMLBuilding building, GMLRoad road,
			List<GMLNode> nodes) {
		List<GMLDirectedEdge> edges = new ArrayList<GMLDirectedEdge>();
		GMLEdge edge;
		HashMap<GMLDirectedEdge, Integer> nei = new HashMap<GMLDirectedEdge, Integer>();
		for (int i = 0; i < nodes.size() - 1; i++) {
			addNode(nodes.get(i));
			if (i != indexIntersection[1] && i != indexIntersection[3]) {
				edge = new GMLEdge(nextID++, nodes.get(i), nodes.get(i + 1),
						false);
				GMLDirectedEdge dEdge = new GMLDirectedEdge(edge, true);
				edges.add(dEdge);
				addEdge(dEdge.getEdge());
				nei.put(dEdge, null);
			}
		}

		// split building edge, remove old one
		splitEdge(crossEdges[0], nodes.get(indexIntersection[0]));
		removeEdge(crossEdges[0]);

		splitEdge(crossEdges[3], nodes.get(indexIntersection[3]));
		removeEdge(crossEdges[3]);
		
		int roadID=(nextID++);

		// fine the connection edges between these two nodes.
		List<GMLEdge> tempEdges=getShortestEdgesbetweenTwoNodes(nodes.get(indexIntersection[0]),nodes.get(indexIntersection[3]));
		for(int i=0;i<tempEdges.size();i++)
		{
			GMLEdge t=tempEdges.get(i);
			t.setPassable(true);
			GMLDirectedEdge de=new GMLDirectedEdge(tempEdges.get(i), true);
			edges.add(de);
			nei.put(de, building.getID());
			building.setNeighbour(t, roadID);
		}

		splitEdge(crossEdges[1], nodes.get(indexIntersection[1]));
		removeEdge(crossEdges[1]);

		splitEdge(crossEdges[2], nodes.get(indexIntersection[2]));
		removeEdge(crossEdges[2]);
		
		tempEdges=getShortestEdgesbetweenTwoNodes(nodes.get(indexIntersection[1]),nodes.get(indexIntersection[2]));
		for(int i=0;i<tempEdges.size();i++)
		{
			GMLEdge t=tempEdges.get(i);
			t.setPassable(true);
			GMLDirectedEdge de=new GMLDirectedEdge(tempEdges.get(i), true);
			edges.add(de);
			nei.put(de, road.getID());
			road.setNeighbour(t, roadID);
		}

		GMLRoad road1=new GMLRoad(roadID, edges);
		road1.setNeighbours(nei);		
		addRoad(road1);
	}
	 ...
	 * @return
	 */
	public List<GMLNode> getGmlNodes(GMLBuilding building, GMLRoad road,
			List<GMLNode> nodes) {
		ArrayList<GMLNode> result = new ArrayList<GMLNode>();
		int index = 0;
		indexIntersection = new int[4];
		crossEdges = new GMLEdge[4];
		for (int i = 0; i < nodes.size() - 1; i++) {
			GMLEdge tempEdge = new GMLEdge(0, nodes.get(i), nodes.get(i + 1),
					false);
			GMLDirectedEdge edge = new GMLDirectedEdge(tempEdge, true);
			GMLObject[] r = getInterSection(building, edge);
			if (r[0] != null) {
				result.add((GMLNode) r[0]);
				indexIntersection[0] = 0;
				crossEdges[0] = (GMLEdge) r[1];
				index = i;
				break;
			}
		}
		for (int i = index; i < nodes.size(); i++) {
			...
			GMLObject[] r = getInterSection(road, edge);
			if (r[0] == null) {
				GMLNode node2 = new GMLNode(nextID++, nodes.get(i + 1).getX(),
						nodes.get(i + 1).getY());
				result.add(node2);
				indexIntersection[1] = indexIntersection[1] + 1;
			} else {
				result.add((GMLNode) r[0]);
				crossEdges[1] = (GMLEdge) r[1];
				indexIntersection[1] = indexIntersection[1] + 1;
				index = i + 1;
				break;
			}
		...
		for (int i = index; i < nodes.size(); i++) {
			...
			if (r[0] != null) {
				result.add((GMLNode) r[0]);
				indexIntersection[2] = indexIntersection[1] + 1;
				crossEdges[2] = (GMLEdge) r[1];
				index = i;
				break;
			}
		}
		for (int i = index; i < nodes.size(); i++) {
			...
			GMLDirectedEdge edge = new GMLDirectedEdge(tempEdge, true);
			GMLObject[] r = getInterSection(building, edge);
			indexIntersection[3] = indexIntersection[2];
			if (r[0] == null) {
				GMLNode node2 = new GMLNode(nextID++, nodes.get(i + 1).getX(),
						nodes.get(i + 1).getY());
				result.add(node2);
				indexIntersection[3] = indexIntersection[3] + 1;
			} else {
				result.add((GMLNode) r[0]);
				indexIntersection[3] = indexIntersection[3] + 1;
				crossEdges[3] = (GMLEdge) r[1];
				break;
			}
		}
		return result;
	...
	/**
	 * get intersection point between the shape and the edge
	 * 
	 * @param shape
	 * @param edge
	 * @return
	 */
	public GMLObject[] getInterSection(GMLShape shape, GMLDirectedEdge edge) {
		List<GMLDirectedEdge> edges = shape.getEdges();
		GMLObject result[] = new GMLObject[2];
		for (int i = 0; i < edges.size(); i++) {
			GMLDirectedEdge edge2 = edges.get(i);
			GMLNode node = getInterSection(edge, edge2);
			// assume that there is only one insection point between the edge
			// and the shape
			if (node != null) {
				result[0] = node;
				result[1] = edges.get(i).getEdge();
				break;
			}
		}
		return result;
	...
	public GMLNode getInterSection(GMLDirectedEdge edge1, GMLDirectedEdge edge2) {
		GMLCoordinates start1 = edge1.getStartCoordinates();
		GMLCoordinates end1 = edge1.getEndCoordinates();
		GMLCoordinates start2 = edge2.getStartCoordinates();
		GMLCoordinates end2 = edge2.getEndCoordinates();

		double a1 = (start1.getY() - end1.getY())
				/ (start1.getX() - end1.getX());
		double b1 = start1.getY() - a1 * start1.getX();
		double a2 = (start2.getY() - end2.getY())
				/ (start2.getX() - end2.getX());
		double b2 = start2.getY() - a2 * start2.getX();

		if (Math.abs(a1 - a2) < 0.00001)// parallel lines
			return null;
		else {
			double x = (b2 - b1) / (a2 - a1);
			double y = a1 * x + b1;
			// insection point not in the limits
			if ((x > start1.getX() && x > end1.getX())
					|| (x < start1.getX() && x < end1.getX())
					|| (x > start2.getX() && x > end2.getX())
					|| (x < start2.getX() && x < end2.getX()))
				return null;
			else {
				return new GMLNode(nextID++, x, y);
			}
		}
	}

	/**
	 * find the shortest edge list between two nodes in the shape
	 * 
	 * @param shape
	 * @param node1
	 * @param node2
	 * @return
	 */
	public List<GMLEdge> getShortestEdgesbetweenTwoNodes(GMLNode node1, GMLNode node2) {
		HashMap<GMLNode, Double> nodesDis = new HashMap<GMLNode, Double>();
		HashMap<GMLNode, List<GMLEdge>> nodesPath = new HashMap<GMLNode, List<GMLEdge>>();
		HashMap<GMLNode, Double> nodesOut = new HashMap<GMLNode, Double>();
		HashMap<GMLNode, GMLNode> nodesPredecessors = new HashMap<GMLNode, GMLNode>();
		nodesPath.put(node1, new ArrayList<GMLEdge>());
		nodesOut.put(node1, 0.0);
		GMLNode nextNode = node1;
		while (true) {
			GMLNode curNode = nextNode;
			Iterator<GMLEdge> edges = getAttachedEdges(curNode).iterator();
			while (edges.hasNext()) {
				GMLEdge e = edges.next();
				if (e.getStart().getID() == curNode.getID()
						|| e.getEnd().getID() == curNode.getID()) {
					double l = getEdgeLength(e);
					GMLNode t = e.getStart().getID() == curNode.getID() ? e
							.getEnd() : e.getStart();
					if (!nodesOut.containsKey(t)) {
						if (!nodesDis.containsKey(t)) {
							nodesDis.put(t, nodesOut.get(curNode).doubleValue()
									+ l);
							nodesPredecessors.put(t, curNode);
						} else {
							if (nodesOut.get(curNode).doubleValue() + l < nodesDis
									.get(t)) {
								nodesDis.put(t, nodesOut.get(curNode)
										.doubleValue() + l);
								nodesPredecessors.put(t, curNode);
							}
						}
			...
			GMLNode shortNode = null;
			double shortL = Double.MAX_VALUE;
			Iterator<GMLNode> ite = nodesDis.keySet().iterator();
			while (ite.hasNext()) {
				GMLNode node = ite.next();
				double v = nodesDis.get(node);
				if (v < shortL) {
					v = shortL;
					shortNode = node;
				}
			}
			nodesOut.put(shortNode, nodesDis.get(shortNode));
			nodesPath.put(
					shortNode,
					new ArrayList<GMLEdge>(nodesPath.get(nodesPredecessors
							.get(shortNode))));
			nodesPath.get(shortNode).add(
					ensureEdge(shortNode, nodesPredecessors.get(shortNode)));

			curNode = shortNode;
			if (shortNode.getID() == node2.getID())
				return nodesPath.get(shortNode);
		}
	}

	public double getEdgeLength(GMLEdge edge) {
		double x1 = edge.getStart().getX();
		double y1 = edge.getStart().getY();
		double x2 = edge.getStart().getX();
		double y2 = edge.getStart().getY();
